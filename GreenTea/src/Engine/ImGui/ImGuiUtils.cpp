...
namespace gte::gui {
	...
	void ShiftCursor(const glm::vec2& offset)
	{
		const ImVec2 cursor = ImGui::GetCursorPos();
		ImGui::SetCursorPos({ cursor.x + offset.x, cursor.y + offset.y });
	}

	ImRect GetItemRect(void) { return ImRect(ImGui::GetItemRectMin(), ImGui::GetItemRectMax()); }

	ImRect RectExpanded(const ImRect& rect, const glm::vec2& size)
	{
		ImRect result = rect;
		result.Min.x -= size.x;
		result.Min.y -= size.y;
	...
	ImRect RectOffset(const ImRect& rect, const glm::vec2& offset)
	{
		ImRect result = rect;
