...
void main(void)
{
	...
	vec4 Position_WCS = u_ModelMatrix * vec4(_position, 1.0f);
	v_Position_WCS = Position_WCS.xyz;
	v_Normal = normalize((u_NormalMatrix * vec4(_normal, 0)).xyz);
	v_TextCoords = _textCoords;
	gl_Position = u_EyeMatrix * Position_WCS;
}
...
{
	// sample shadow map
	float shadow_map_z = texture2D(ShadowmapTexture, light_space_xyz.xy).r;

	// + shaded -> 0.0 
	// - lit -> 1.0
	// < closer to the light
	return ((light_space_xyz.z - u_ConstantBias) < shadow_map_z) ? 1.0 : 0.0;
}
...
float shadow_pcf2x2_weighted(vec3 light_space_xyz)
{
	...
	float z = u_LightPos.z - u_ConstantBias;
...
vec4 blinn_phong(const in vec3 pSurfToEye, const in vec3 pSurfToLight)
{
	...
	if(u_HasNormal)
	{
		...
		if(u_IsBump)
		{
			float heighPrevU = textureOffset(NormalTexture, v_TextCoords, ivec2(-1, 0)).r;
			float heighPrevV = textureOffset(NormalTexture, v_TextCoords, ivec2(0, -1)).r;
			normal = normal - v_TBN[0] * (nmap.r - heighPrevU) - v_TBN[1] * (nmap.r - heighPrevV);
		}
		else
		{
			nmap = nmap * 2.0 - 1.0;
	...
	vec4 specular = NdotL > 0.0 ? ks * fn * pow(NdotH, u_Shininess) : vec4(0.0, 0.0, 0.0, 1.0);
...
void main()
{
	if(u_HasLight)
	{
		...
		vec4 brdf = vec4(1.0, 0.0, 0.0, 1.0);
		vec4 DiffuseColor = vec4(u_Diffuse.rgb, 1.0);
		if(u_HasTexture)
			DiffuseColor *= texture(DiffuseTexture, v_TextCoords);
		o_Color = DiffuseColor;//o_Color = vec4(shadow_value * brdf.rgb, 1.0);
	}
	else
	{
		vec4 DiffuseColor = vec4(u_Diffuse.rgb, 1.0);
